...
//	jrsbList.js is a component of the "Slide Box" part of "Multifocal-plane Virtual Microscope"
...
//	jrsbList.js is part of the "SlideBox"
...
function gotoViewer(nodeId,musEvt) {
	...
	if ((glbInSViewerURL == null) || (glbInSViewerURL == "")) {
		alert("gotoViewer():  cannot go \"to microscope\" because the URL for the Viewer is not defined."
					+ "\n\nPlease report this error.");
		return;
		}
	var urlSldBox = glbInSViewerURL + "?sb";
	...
	else if (isCntrlKey) { window.location.replace(urlSldBox); }
	else { window.location.assign(urlSldBox); }
	return;
	}
...
function lstWndBuild() {
	...
	txtStr += "to view the slide in the Virtual Microscope.";
